  ...
  return (
    <div className="mt-4 w-full">
      {sharedState?.chunks.map((item: any, i: number) => (
        <div key={i} className=" my-6 flex flex-col items-start  bg-gradient-to-br from-main-pink  border-2 border-black p-2 rounded-md w-[100%] shadow-[0_4px_1px_1px_rgba(0,0,0,0.3)]">
          {item.type === "Table" ? (
            <div dangerouslySetInnerHTML={{ __html: item.text }} />
          ) : (
            <div>

              <p className="  ml-2 font-semibold text-[1.25rem]">{item.text}</p><p className="  ml-2 font-semibold text-[1.25rem]">{item.text}</p>
...
{/* <h4 className=' font-medium text-[1.25rem]' >{`Chunk ${item.id+1}.`}</h4> */ }
